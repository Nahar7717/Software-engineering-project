...
class SecurityProvider {
    ...
    private static final byte[] SECRET_KEY = "RideShareSecureK".getBytes();

    public static String encryptAsset(String data) {
        try {
            javax.crypto.spec.SecretKeySpec keySpec = new javax.crypto.spec.SecretKeySpec(SECRET_KEY, ALGORITHM);
...
class RideRequest {
    ...
    public String toString() {
        return "<html><b>" + from + " -> " + to + "</b><br>Time: " + time + " | Requested by: " + student.getName()
                + "</html>";
    }
}
...
class User {
    ...
    public void submitDriverDocs(String license, String iqama, String dob, String car) {
        // Encrypt sensitive IDs before saving them
        this.licenseId = SecurityProvider.encryptAsset(license);
        this.iqamaId = SecurityProvider.encryptAsset(iqama);
        this.birthDate = dob;
        this.carModel = car;
    ...
    public void rejectDriver() {
        this.driverStatus = DriverStatus.REJECTED;
    }
...
class Ride {
    ...
    public Ride(String driverName, String driverPhone, String from, String to, double cost, RideType type,
            String dateTime) {
    ...
    public String toString() {
        ...
        return "<html><b style='font-size:110%'>[" + typeIcon + "] " + from + " -> " + to + "</b><br>" +
                "<span style='color:gray'>" + dateTime + " | " + cost + " SAR | Driver: " + driverName
                + "</span></html>";
    }
    ...
    public String getDriverViewString() {
        ...
        if (status == RideStatus.BOOKED) {
            stateHtml = "<span style='color:cyan'>BOOKED by: " + passenger.getName() + " | Ph: " + passenger.getPhone()
                    + "</span>";
...
public class RideSharing extends JFrame {
    ...
    public RideSharing() {
        ...
        allRides.add(new Ride(demoDriver.getName(), demoDriver.getPhone(), "Al-Nakhil Mall", "IMAMU Gate 1", 25.0,
                RideType.INSTANT, "NOW"));
        allRides.add(new Ride(demoDriver.getName(), demoDriver.getPhone(), "Airport", "IMAMU Housing", 40.0,
                RideType.SCHEDULED, "15-12-2025 09:00 AM"));
    ...
    private void refreshLists() {
        ...
        for (User u : users) {
            if (u.getDriverStatus() == DriverStatus.PENDING)
                pendingDriversModel.addElement(u);
        }
    }
    ...
    private JPanel createLoginPanel() {
        ...
        gbc.gridy = 3;
        panel.add(new JLabel("ID / Username:"), gbc);
        gbc.gridy = 4;
    ...
    private void attemptLogin() {
        ...
        User foundUser = null;
        for (User u : users) {
            if (u.getId().equals(id) && u.checkPassword(pass)) {
                foundUser = u;
                break;
            }
        }
        ...
        if (foundUser == null) {
            JOptionPane.showMessageDialog(this, "User not found or wrong password.");
            return;
        }
        currentUser = foundUser;

        if (role.equals("Driver Login")) {
            if (currentUser.getDriverStatus() == DriverStatus.APPROVED)
                cardLayout.show(mainPanel, "DriverDash");
            else if (currentUser.getDriverStatus() == DriverStatus.PENDING)
                JOptionPane.showMessageDialog(this, "Pending Admin Approval.");
            else if (currentUser.getDriverStatus() == DriverStatus.REJECTED)
                JOptionPane.showMessageDialog(this, "Rejected. Login as Student to re-apply.");
            else
                JOptionPane.showMessageDialog(this, "Not a driver. Login as Student to apply.");
        } else {
    ...
    private JPanel createRegisterPanel() {
        ...
        gbc.gridx = 0;
        gbc.gridy = 2;
        panel.add(new JLabel("Name (Letters Only):"), gbc);
        gbc.gridx = 1;
        panel.add(regNameField, gbc);
        gbc.gridx = 0;
        gbc.gridy = 3;
        panel.add(new JLabel("Phone (05...):"), gbc);
        gbc.gridx = 1;
        ...
        doRegBtn.addActionListener(e -> {
            ...
            if (!isValidPassword(pass)) {
                JOptionPane.showMessageDialog(this, "Password Invalid! 8+ chars, Upper, Lower, Special.");
                return;
            }
    ...
    private JPanel createStudentDashboard() {
        ...
        myRidesBtn.addActionListener(e -> {
            refreshLists();
            cardLayout.show(mainPanel, "StudentHistory");
        });
        verifyBtn.addActionListener(e -> {
            if (currentUser.getDriverStatus() == DriverStatus.APPROVED)
                JOptionPane.showMessageDialog(this, "You are already a driver!");
            else if (currentUser.getDriverStatus() == DriverStatus.PENDING)
                JOptionPane.showMessageDialog(this, "Application Pending.");
            else
                cardLayout.show(mainPanel, "Verification");
        });
        emergencyBtn.addActionListener(
                e -> JOptionPane.showMessageDialog(this, "Emergency Alert Sent! Authorities have been notified."));
    ...
    private JPanel createStudentMyRidesPanel() {
        ...
        payBtn.addActionListener(e -> {
            ...
            if (r != null && r.status == RideStatus.BOOKED) {
                int result = JOptionPane.showOptionDialog(this, "Total: " + r.cost + " SAR. Select payment method:",
                        "Payment", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null,
                        new Object[] { "Card", "Wallet", "Cash", "Cancel" }, "Card");
                if (result == 3 || result == JOptionPane.CLOSED_OPTION)
                    return; // Cancel
                r.finishRide();
                String paymentMsg;
                if (result == 0)
                    paymentMsg = "Card payment processed! Ride Finished.";
                else if (result == 1)
                    paymentMsg = "Wallet payment processed! Ride Finished.";
                else
                    paymentMsg = "Please pay the driver " + r.cost + " SAR in cash upon pickup. Ride Finished.";
                    ...
                    try {
                        ...
                        if (rating >= 1 && rating <= 5) {
                            ...
                            String review = JOptionPane.showInputDialog(this,
                                    "Leave a review for your driver (optional):");
                            if (review != null) {
                                r.review = review;
                            }
                        } else {
                ...
                JOptionPane.showMessageDialog(this, "Ride is already finished or invalid.");
            }
        });
        ...
        cancelBtn.addActionListener(e -> {
            ...
            Ride r = findRideByPassenger(currentUser, idx);
            if (r != null && r.status == RideStatus.BOOKED) {
                int confirm = JOptionPane.showConfirmDialog(this, "Cancel this ride?", "Cancel",
                ...
                JOptionPane.showMessageDialog(this, "Cannot cancel finished rides.");
            }
        });
    ...
    private JPanel createDriverDashboard() {
        ...
        myRidesBtn.addActionListener(e -> {
            refreshLists();
            cardLayout.show(mainPanel, "DriverHistory");
        });
        emergencyBtn.addActionListener(
                e -> JOptionPane.showMessageDialog(this, "Emergency Alert Sent! Authorities have been notified."));
        profileBtn.addActionListener(e -> cardLayout.show(mainPanel, "Profile"));
    ...
    private JPanel createVerificationPanel() {
        ...
        JTextField iqamaField = new JTextField();
        iqamaField.setPreferredSize(fieldSize);
        JTextField dobField = new JTextField();
        dobField.setPreferredSize(fieldSize);
        JTextField carField = new JTextField();
        carField.setPreferredSize(fieldSize);
        ...
        gbc.gridy++;
        panel.add(new JLabel("License Number (Nums):"), gbc);
        gbc.gridy++;
        panel.add(licenseField, gbc);

        gbc.gridy++;
        panel.add(new JLabel("ID / Iqama Number (Nums):"), gbc);
        gbc.gridy++;
        panel.add(iqamaField, gbc);

        gbc.gridy++;
        panel.add(new JLabel("Birth Date (DD/MM/YYYY):"), gbc);
        gbc.gridy++;
        ...
        JButton backBtn = createButton("Back", Color.GRAY);
        JButton submitBtn = createButton("Submit", IMAMU_GREEN);
        btnPanel.add(backBtn);
        ...
        submitBtn.addActionListener(e -> {
            ...
            if (!isNumeric(licenseField.getText()) || !isNumeric(iqamaField.getText())) {
                JOptionPane.showMessageDialog(this, "License and ID must be NUMBERS ONLY.");
                return;
            }
            currentUser.submitDriverDocs(licenseField.getText(), iqamaField.getText(), dobField.getText(),
                    carField.getText());
    ...
    private void showDriverAppsDialog() {
        ...
        list.setCellRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
                ...
                User u = (User) value;
                setText(u.getName() + " (ID: " + u.getId() + ")");
                return this;
            }
        });
        int result = JOptionPane.showConfirmDialog(this, new JScrollPane(list), "Select Driver to Verify",
                JOptionPane.OK_CANCEL_OPTION);
        if (result == JOptionPane.OK_OPTION && list.getSelectedValue() != null) {
            User u = list.getSelectedValue();
            int action = JOptionPane.showOptionDialog(this, u.getDriverDetails(), "Approve?",
                    JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null,
                    new Object[] { "Approve", "Reject", "Cancel" }, "Approve");
            if (action == 0) {
                u.approveDriver();
            ...
            if (idx != -1) {
                ...
                if (r != null) {
                    ...
                    JOptionPane.showMessageDialog(this,
                            "Booked! Please go to 'My Rides' to Pay or Cancel.\nDriver Phone: " + r.driverPhone);
                    refreshLists();
                    cardLayout.show(mainPanel, "StudentDash");
                }
            } else {
    ...
    private JPanel createOfferRidePanel() {
        ...
        from.setPreferredSize(fieldSize);
        JTextField to = new JTextField();
        to.setPreferredSize(fieldSize);
        JTextField cost = new JTextField();
        cost.setPreferredSize(fieldSize);
        ...
        panel.add(to, gbc);
        gbc.gridy++;
        panel.add(new JLabel("Cost (SAR):"), gbc);
        gbc.gridy++;
        panel.add(cost, gbc);
        gbc.gridy++;
        panel.add(isScheduled, gbc);
        gbc.gridy++;
        ...
        gbc.gridy++;
        JPanel btnPanel = new JPanel(new GridLayout(1, 2, 10, 0));
        JButton back = createButton("Back", Color.GRAY);
        JButton post = createButton("Post", IMAMU_BLUE);
        btnPanel.add(back);
        btnPanel.add(post);
        btnPanel.setPreferredSize(fieldSize);
        panel.add(btnPanel, gbc);
        ...
        post.addActionListener(e -> {
            try {
                ...
                allRides.add(new Ride(currentUser.getName(), currentUser.getPhone(), from.getText(), to.getText(),
                        Double.parseDouble(cost.getText()), type, t));
                refreshLists();
    ...
    private JPanel createProfilePanel() {
        ...
        saveBtn.addActionListener(e -> {
            ...
            cardLayout.show(mainPanel,
                    currentUser.getDriverStatus() == DriverStatus.APPROVED ? "DriverDash" : "StudentDash");
        });
        backBtn.addActionListener(e -> cardLayout.show(mainPanel,
                currentUser.getDriverStatus() == DriverStatus.APPROVED ? "DriverDash" : "StudentDash"));
        return panel;
    }
    ...
    private JPanel createRequestRidePanel() {
        ...
        JPanel btnPanel = new JPanel(new GridLayout(1, 2, 10, 0));
        JButton back = createButton("Back", Color.GRAY);
        JButton request = createButton("Post Request", IMAMU_BLUE);
        btnPanel.add(back);
